      ...
      <div className="flex-1 scroll-area px-6 pb-6 space-y-4">
        ...
        <div className="glass-sm px-5 py-5 space-y-3">
          ...
          <p className="text-white/35 text-xs leading-relaxed">
            Delete all sessions for a specific date. Requires password. This cannot be undone.
          </p>
          {!showDelete ? (
            <button
              onClick={() => setShowDelete(true)}
              className="btn-ghost w-full text-center text-sm py-3 text-red-400 border-red-500/20"
              style={{ borderColor: 'rgba(239,68,68,0.2)' }}
            >
              Delete a Day's Sessions
            </button>
          ) : (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => { setShowDelete(false); setDeleteError(''); setDeleteDate(''); setDeletePass('') }}
                  className="flex-1 btn-ghost text-center text-sm py-3"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={deleting}
                  className="flex-1 py-3 rounded-2xl font-semibold text-sm text-red-400 border border-red-500/30 bg-red-500/10 active:scale-95 disabled:opacity-60 transition-all"
                >
